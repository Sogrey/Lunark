...
| 一期 MVP（方案 B 双栏） | 见 [ROADMAP.md](./ROADMAP.md) | **完成（含打磨，可进二期）** |
| 二期 | WYSIWYG 一体化 | **未开始** |
| 三期 | 会话 / 全局搜索 / Vim | **未开始** |
...
| 原生菜单 + 快捷键（防双触发） | ✅ | `appMenu` + Toolbar |
| Ctrl+Tab 切标签 | ✅ | Toolbar |
...
进入 **二期 WYSIWYG**（见 [PHASE2-WYSIWYG.md](./PHASE2-WYSIWYG.md)）。  
未做（三期）：Tabs 会话记忆、全局搜索、Vim。
